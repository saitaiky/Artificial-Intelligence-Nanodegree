...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        a_nodes = []
        # self.all_actions: Eat(Cake,), Bake(Cake,), Noop_pos(Have(Cake),), etc...
        for action in self.all_actions:
            # Build an action Node for this action
            node_a = PgNode_a(action)

            # -----Object 1: determine what actions to add and create those PgNode_a objects-----
            # For this action node to be reachable, its preconditions must be satisfied by the previous state level.
            # 要知有咩Action 行得，就要尼個Action既precondition  滿足到上一個State Level
            # 即是node_a.prenodes 要係self.s_levels[level]入面。 Note: S0 set of s_nodes - empty to start
            if node_a.prenodes.issubset(self.s_levels[level]):
                # -----Object 2: connect the nodes to the previous S literal level-----
    ...
    def add_literal_level(self, level):
        ...
        # determine all possible literal S nodes based on effects(Result) in A level
    ...
    def update_a_mutex(self, nodeset):
        ...
        # Sai: This part loop throguh all sibling in the same level
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # EG: node_s1.symbol = Eaten(Cake), node_s2.symbol = Have(Cake)
        # EG: node_s1.is_pos = False, node_s2.is_pos = True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal_expr in self.problem.goal: #EG: goal_expr = Have(Cake)
            #print("=============Goal===============")
            #print(goal_expr)

            for idx, node_state_set in enumerate(self.s_levels):
                node_state = PgNode_s(goal_expr, True) #EG: node_state = Have(Cake)
                #print("---------state---------")
                #print(node_state.symbol)
                #print(node_state.show())
                if node_state in node_state_set:
                    level_sum = level_sum + idx
                    #print("End! The first node to meet our goal has appeared!")
                    # stop, it first appears in the planning graph
                    break
        return level_sum
