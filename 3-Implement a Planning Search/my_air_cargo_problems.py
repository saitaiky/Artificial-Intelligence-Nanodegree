...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        Sai: Which means we are gonna just call this function once in the constructor!
        ...
        def load_actions():
            ...
            These 2 variable are the results of an action instance that add states to the succeeding level in the graph.
            Rem is for removed, as in the literal that is removed from the pos literal list.
            ========================Or just imagine===================================
            effect_add = positive result from 謂詞(predicate)
            effect_rem = negative result from 謂詞(predicate)
            """
            loads = []

            # Create every cargos, planes and airports based on the problem we are gonna deal with
            # then create all the actions for the problem domain(posibilities).
            # Like: Load(C1, P1, JFK), Load(C1, P1, SFO) etc...
        ...
        def unload_actions():
            ...
            # Create every cargos, planes and airports based on the problem we are gonna deal with
            # then create all the actions for the problem domain(posibilities).
            # Like: Unload(C1, P1, JFK), Unload(C1, P1, SFO) etc...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        precond_pos = [
        ...
        def fly_actions():
            ...
            """
            flys = []
            for fr in self.airports: # Sai: From airpot
                for to in self.airports: # Sai: To airpot
                    if fr != to:
    ...
    # def f(x) -> list: is a Function Annotations sytnax
    # Whcih mean this function can only return list
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        Also, this class will eventually call
        def run_search(problem, search_function, parameter=None): in run_search.py then
        def astar_search(problem, h=None): in search.py, noted that, h arg will pass in our heuristic function
        like, h_1, h_ignore_preconditions and h_pg_levelsum in this example.
        """
        ...
        for clause in self.goal:
            if clause not in kb.clauses:
                # Sai: Instead of return T/F,
                # we return a value based on how many fluents(state variables) of current state are in goal state
                # A star search will then find the minimum number of actions to motivate our agent move toward
                # that direction.
                count += 1
        return count
...
def air_cargo_p1() -> AirCargoProblem:
    ...
    neg = [expr('At(C1, JFK)'), # Start the cargo and location relation
           expr('At(C2, SFO)'),
           expr('At(P1, JFK)'), # Start the plane and location relation
           expr('At(P2, SFO)'),
           expr('In(C1, P2)'), # Start the cargo and plane relation
           expr('In(C2, P1)'),
           expr('In(C1, P1)'),
           expr('In(C2, P2)'),
           ]
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    neg = [expr('At(C1, JFK)'), # Start the cargo and location relation
           ...
           expr('At(P3, JFK)'),
           expr('At(P3, SFO)'),
           expr('In(C1, P1)'), # Start the cargo and plane relation
           expr('In(C1, P2)'),
           expr('In(C1, P3)'),
           expr('In(C2, P1)'),
           expr('In(C2, P2)'),
